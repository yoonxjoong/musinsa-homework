...
<template>
  <div class="relative min-h-screen p-6">
    ...
    <div class="p-6 bg-gray-100 min-h-screen">
      <!-- 카테고리 제목 -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold mb-2">검색된 카테고리 : {{ resultCategoryNm }}</h1>
      </div>

      <!-- 최저가 정보 -->
      <div v-if="lowestPriceCommodity" class="mb-6 bg-white p-4 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-2">최저가 : {{lowestPriceCommodity.price}}</h2>
        <ul>
          <li class="font-medium">브랜드 : {{lowestPriceCommodity.brandNm}}</li>
        </ul>
      </div>
      <div v-else class="mb-6 bg-white p-4 rounded-lg shadow-md">
        <p class="text-lg font-semibold text-red-500">최저가 정보가 없습니다.</p>
      </div>

      <!-- 최고가 정보 -->
      <div v-if="highestPriceCommodity" class="bg-white p-4 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-2">최고가 : {{highestPriceCommodity.price}}</h2>
        <ul>
          <li class="font-medium">브랜드 : {{highestPriceCommodity.brandNm}}</li>
        </ul>
      </div>
